class Source extends React.Component {
  ...
  render() {
    ...
    let source_type = this.state.editable ? (
      <input
        type="text"
        ref={input => (this.source_type = input)}
        defaultValue={this.props.source.source_type}
      />
    ) : (
      <p>{this.props.source.source_type}</p>
    )
    return (
      <div>
        {title}
        ...
        <button onClick={() => this.props.handleDelete(this.props.source.id)}>
          Delete
        </button>
      </div>
    )
